...
function mapInsight(row: MetaAdInsight, adAccountId: string, syncedAt: string) {
  ...
  return {
    ...
    meta_purchase_value: metricValue(row.action_values, PURCHASE_ACTIONS),
...
async function insertRows(tableName: string, rows: Record<string, unknown>[]) {
  ...
  for (let i = 0; i < rows.length; i += 500) {
    await table.insert(rows.slice(i, i + 500), { raw: false, ignoreUnknownValues: true });
  }
}
...
export async function GET() {
  try {
    ...
    if (!accessToken || !adAccountId) {
      return NextResponse.json({ ok: false, error: 'META_ADS_ACCESS_TOKEN and META_AD_ACCOUNT_ID are required' }, { status: 500 });
    }

    const syncedAt = new Date().toISOString();
    const client = createBigQueryClient(PROJECT_ID, LOCATION);
    const [insights, creatives] = await Promise.all([
      fetchMetaAdInsights({ accessToken, adAccountId, datePreset: 'yesterday' }),
      fetchMetaAdCreatives({ accessToken, adAccountId }),
    ]);

    const insightRows = insights.filter((row) => row.ad_id && row.date_start && row.date_stop).map((row) => mapInsight(row, adAccountId, syncedAt));
    if (insightRows.length > 0) {
      const dates = insightRows.map((row) => String(row.date_start)).sort();
      await client.query({
        query: `
        ...
        `,
        params: { adAccountId, startDate: dates[0], endDate: dates[dates.length - 1] },
      });
