...
import type { BenchmarkRating, ReelMetricRow, ReelMetricsDashboardData } from '@/lib/instagram/reelMetricsDashboard';
  ...
  return (
    <div className="flex items-start gap-4 rounded-[var(--radius-md)] border border-[color:var(--color-border)] bg-white p-3">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-2">
        ...
        {snapshot.dropoffSegment && (
          <div className="rounded-md border border-[color:var(--color-border)] bg-[color:var(--color-surface-muted)] p-2 text-xs">
            <div className="text-[color:var(--color-text-muted)]">
              平均 {snapshot.avgWatchTimeSeconds?.toFixed(1)}秒で離脱 →
              <span className="ml-1 text-[color:var(--color-text-muted)]">
                [{snapshot.dropoffSegment.start.toFixed(1)}s〜{snapshot.dropoffSegment.end.toFixed(1)}s]
              </span>
            </div>
            <div className="mt-1 font-medium text-[color:var(--color-text-primary)]">
              「{snapshot.dropoffSegment.text}」
            </div>
          </div>
        )}
