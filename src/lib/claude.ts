...
const JSON_SCHEMA_EXAMPLE = `{
  "post": {
    ...
    "comments": ["僕も最初は...\\n\\n具体的な使い方はこんな感じ▼\\n\\n【ステップ1】...\\n【ステップ2】...\\n\\nこれで作業時間が激減しました。（必ず400文字以上、最大500文字。適切な箇所で改行を入れる）", "さらに効果を爆上げする...\\n\\n具体的には...\\n\\nただし注意点が3つ▼\\n①...\\n②...\\n③...\\n\\n今すぐ試してみてください。（必ず400文字以上、最大500文字。適切な箇所で改行を入れる）"]
  }
}`;
...
async function buildBatchContext(payload: ThreadsPromptPayload): Promise<string> {
  ...
  return [
    ...
    '      "comments": ["[コメント欄1: 必ず400文字以上・最大500文字、段落ごとに\\n\\nで改行]", "[コメント欄2: 必ず400文字以上・最大500文字、段落ごとに\\n\\nで改行]"]',
    '    },',
    '    {',
    '      "planId": "[plan-02など]",',
    '      ...',
    ...
    '- **文字数厳守**: mainPost 150-200文字、comments 必ず400文字以上（最大500文字）',
    '- **超重要**: コメントは必ず400文字以上。300文字以下は絶対NG。500文字超過も厳禁。',
    '- **改行必須**: 各文字列内で適切な箇所に\\n\\nを入れて読みやすくする',
  ].join('\n');
}
